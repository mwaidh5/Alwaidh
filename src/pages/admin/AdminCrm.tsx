...
function openPhoto(url: string) {
  ...
  window.open(url, cap?.isNativePlatform?.() ? '_system' : '_blank');
}
...
function CardBody({ contact, onView }: { contact: CrmContact; onView?: (c: CrmContact) => void }) {
  ...
  return (
    ...
    >
      ...
      {contact.phone && (
        <div className="mt-2 flex items-center gap-1.5" dir="ltr" onClick={(e) => e.stopPropagation()}>
          <a
        ...
        {shown.map((s) =>
          byStatus[s.key].map((c) => (
            ...
            >
              ...
              {c.phone && (
                <div className="mt-1.5 flex items-center gap-2" dir="ltr" onClick={(e) => e.stopPropagation()}>
                  <a
              ...
              <label className="mt-2.5 block" onClick={(e) => e.stopPropagation()}>
...
function ContactDetails({
  ...
  async function postNote() {
    ...
    const files = photos;
    if ((!text && !files.length) || posting) return;
    setPosting(true);
    try {
      ...
      setNote('');
      files.forEach((ph) => URL.revokeObjectURL(ph.url));
      setPhotos([]);
    } catch (e) {
          ...
          <div>
                      ...
                      <div className={`${n.text ? 'mt-2' : ''} flex flex-wrap gap-1.5`}>
                        {n.images.map((u) => (
                          <button
                            ...
                            type="button"
                            onClick={() => openPhoto(u)}
                            className="h-20 w-20 overflow-hidden rounded-lg bg-white ring-1 ring-slate-200"
                          >
            ...
            <div className="mt-2 flex items-end gap-2">
              <button
                ...
                title={t('Photo')}
                aria-label={t('Photo')}
                className="grid h-10 w-10 flex-none place-items-center rounded-xl border border-slate-300 bg-white text-lg hover:bg-slate-50 disabled:opacity-50"
              >
                📷
              </button>
              <textarea
